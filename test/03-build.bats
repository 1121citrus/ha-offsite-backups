...
@test "build outputs Trivy volume name at startup" {
    run "${BUILD_SCRIPT}" --dry-run --no-lint --no-test --no-scan --no-advise 2>&1
    [[ $status -eq 0 ]]
    [[ "$output" == *"Trivy DB:  volume:ha-offsite-backups-trivy-cache"* ]]
}

@test "build outputs Grype volume name at startup" {
    run "${BUILD_SCRIPT}" --dry-run --no-lint --no-test --no-scan --no-advise 2>&1
    [[ $status -eq 0 ]]
    [[ "$output" == *"Grype DB:  volume:ha-offsite-backups-grype-cache"* ]]
}
...
@test "build --advise grype does not re-prime a populated Grype cache" {
    ...
    docker volume rm ha-offsite-backups-grype-cache >/dev/null 2>&1 || true
    docker volume create ha-offsite-backups-grype-cache >/dev/null
...
@test "build --version sets image tag" {
    ...
    [[ $status -eq 0 ]]
    [[ "$output" == *"1121citrus/ha-offsite-backups:1.2.3"* ]]
}
...
@test "build --registry sets registry" {
    ...
    [[ $status -eq 0 ]]
    [[ "$output" == *"myregistry/ha-offsite-backups"* ]]
}
